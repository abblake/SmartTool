...
#initial lazy pull
industry_pull <- tbl(wrds, sql("select lpad(sich::text, 4,'0') as SICH, fyear as YEAR, at, ebit, gvkey
...
#now average
industry_pull <- industry_pull %>% group_by(sic_2, year) %>% filter(!is.infinite(roa_temp)) %>% mutate(ind_roa3 = mean(roa_temp, na.rm=T)) %>% ungroup()
