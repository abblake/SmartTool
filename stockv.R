...
#calculate tsr
tsr_calc <- crsp %>% rowwise(permno, year) %>% filter(date==max(date)) %>% mutate(stock_vol1 = sd(c(ret,ret_l1,ret_l2,ret_l3,ret_l4,ret_l5,ret_l6,ret_l7,ret_l8,ret_l9,ret_l10,ret_l11)) %>% ungroup()
#isolate specific variables.
tsr_calc <- tsr_calc %>% select(cusip, permno, permco, date, year, stock_vol1)
